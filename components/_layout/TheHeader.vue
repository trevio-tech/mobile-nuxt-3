<template>
  <header class="h-[48px] flex-shrink-0 flex items-center bg-white shadow-sm sticky top-0">
    ...
    <NuxtLink v-if="! $auth.loggedIn" to="/login" class="flex items-center ml-auto px-4 h-full">Войти</NuxtLink>
    <DropdownMenu v-else placement="bottom-end" class="ml-auto h-full">
      <template v-slot:default="{ isActive }">
        <div class="flex items-center justify-center h-full px-4 overflow-hidden max-w-[180px]" :class="{'bg-slate-100': isActive}">
